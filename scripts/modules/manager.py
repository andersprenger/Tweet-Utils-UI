...
class TokenManager:
    ...
    def test_api(self, api_key, api_secret, access_token, access_token_secret):
        ...
        api = tweepy.API(auth, retry_count=3, retry_delay=5, timeout=100, wait_on_rate_limit=True,
    ...
    def first_cfg_api(self):
        print(
            'Looks like you have not configured your API credentials yet. See https://developer.twitter.com/ for '
            'details\n')
    ...
    def init_api(self):
        try:
            with open('config/config.json') as f:
                token = json.load(f)
            api = self.test_api(token['api_key'], token['api_secret'], token['access_token'],
